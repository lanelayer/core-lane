...
test_block_system() {
    ...
    if [ "$full_found" != "null" ]; then
        ...
        # Check extra data contains "CORE-MEL"
        local extra_data=$(echo "$full_response" | jq -r '.result.extraData')
        if [[ "$extra_data" == *"434f52452d4d454c"* ]]; then
            print_success "   ✅ Extra data contains 'CORE-MEL' identifier"
        else
            print_warning "   ⚠️  Extra data doesn't contain expected 'CORE-MEL' identifier: $extra_data"
        fi
    else
