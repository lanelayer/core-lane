...
impl RpcServer {
    ...
    async fn handle_request(
        ...
        let response = match request.method.as_str() {
            ...
            "eth_maxPriorityFeePerGas" => Self::handle_max_priority_fee_per_gas(&state, request).await,
    ...
    async fn handle_fee_history(
        ...
        let newest_block = request.params[1].as_str().unwrap_or("latest");
        ...
        let base_fee_history = state.eip1559_fee_manager.get_base_fee_history(start_block, end_block);
        ...
        for block_num in start_block..=end_block {
            if let Some(base_fee) = state.eip1559_fee_manager.get_base_fee_for_block(block_num) {
                ...
                if let Some(block) = state.blocks.get(&block_num) {
                    let gas_used_ratio = state.eip1559_fee_manager.get_gas_used_ratio(block.gas_used);
