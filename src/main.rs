...
async fn main() -> Result<()> {
    ...
    match &cli.command {
        ...
        Commands::Start {
            ...
            let mnemonic_str_opt =
                resolve_mnemonic(mnemonic.as_deref(), mnemonic_file.as_deref()).ok();
            ...
            let (shared_state, bitcoin_client_write, node_opt) =
                if sequencer_rpc_url.is_some() && mnemonic_str.is_empty() {
                    ...
                    (Arc::clone(&node.state), None, Some(node))
                } else {
                    ...
                    let node =
                        CoreLaneNode::new(read_client, write_client, cli.data_dir.clone(), network);
