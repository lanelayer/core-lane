...
impl CoreLaneNode {
                    ...
                    Ok(mut parsed_block) => {
                        ...
                        match self.process_block(parsed_block).await {
                            Ok(core_lane_block_number) => {
                                ...
                                state
                                    .bitcoin_height_to_hash
                                    .insert(height, espresso::block_id_from_height(height));
                                state
                                    .bitcoin_height_to_core_block
                                    .insert(height, core_lane_block_number);
                            }
    ...
    /// Fetch burns from new Core Lane blocks (for Espresso mode)
    async fn fetch_core_lane_burns(
        &self,
        core_rpc_url: &str,
        chain_id: u32,
        last_height: &mut Option<u64>,
    ) -> Vec<block::CoreLaneBurn> {
        ...
        for height in start_height..=latest_height {
            match derived::fetch_burns_from_core_lane_block(core_rpc_url, height, chain_id).await {
                ...
                Err(e) => {
                    warn!("Failed to fetch burns from Core Lane block {}: {}", height, e);
                    break;
                }
            }
        }
