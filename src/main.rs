...
impl CoreLaneNode {
    ...
    fn is_core_lane_envelope(&self, script: &Script) -> bool {
        ...
        let first = instr.next().and_then(|r| r.ok());
        println!("       First instruction: {:?}", first);
        if first != Some(Instruction::Op(OP_FALSE)) {
            println!("       ❌ First instruction is not OP_FALSE");
            return false;
        }

        let second = instr.next().and_then(|r| r.ok());
        println!("       Second instruction: {:?}", second);
        if second != Some(Instruction::Op(OP_IF)) {
            println!("       ❌ Second instruction is not OP_IF");
            return false;
        }
        ...
        loop {
            let next_instr = instr.next().and_then(|r| r.ok());
            instruction_count += 1;
            println!(
                "       Instruction {}: {:?}",
                instruction_count + 2,
            ...
            match next_instr {
                Some(Instruction::Op(OP_ENDIF)) => break,
                Some(Instruction::PushBytes(b)) => {
                    println!("       Found push bytes: {} bytes", b.len());
                    data.extend_from_slice(b.as_bytes());
                }
                _ => {
                    println!("       ❌ Unexpected instruction: {:?}", next_instr);
                    return false;
                }
            }
        }
        ...
        if last != Some(Instruction::Op(OP_TRUE)) {
            println!("       ❌ Last instruction is not OP_TRUE");
            return false;
        }
    ...
    fn is_core_lane_envelope_bytes(&self, bytes: &[u8]) -> bool {
        ...
        // Check if bytes start with our envelope pattern: OP_FALSE OP_IF
        if bytes.len() < 2 {
            println!("       ❌ Bytes too short");
            return false;
        }
        ...
        if bytes[0] != 0x00 {
            // OP_FALSE
            println!(
                "       ❌ First byte is not OP_FALSE (0x00), got: 0x{:02x}",
                bytes[0]
            );
            return false;
        }
        ...
        if bytes[1] != 0x63 {
            // OP_IF
            println!(
                "       ❌ Second byte is not OP_IF (0x63), got: 0x{:02x}",
                bytes[1]
            );
            return false;
        }
        ...
        for i in 2..bytes.len().saturating_sub(core_lane_bytes.len()) {
            if &bytes[i..i + core_lane_bytes.len()] == core_lane_bytes {
                ...
                for j in (eth_tx_start + 20)..bytes.len().saturating_sub(2) {
                    if bytes[j] == 0x68 && bytes[j + 1] == 0x51 {
                        // OP_ENDIF OP_TRUE
                        let eth_tx_data = &bytes[eth_tx_start..j];
                        println!(
                            "       ✅ Found OP_ENDIF OP_TRUE at offset {}, extracted {} bytes",
                            j,
                            eth_tx_data.len()
                        );
                        return Some(eth_tx_data.to_vec());
                    }
                }

                // Fallback: look for padding patterns
                for j in (eth_tx_start + 20)..bytes.len().saturating_sub(4) {
                    if bytes[j] == 0xf0
                        && bytes[j + 1] == 0xf0
                        && bytes[j + 2] == 0xf0
                        && bytes[j + 3] == 0xf0
                    {
                        let eth_tx_data = &bytes[eth_tx_start..j];
                        println!(
                            "       ✅ Found padding at offset {}, extracted {} bytes",
                            j,
                            eth_tx_data.len()
                        );
                        return Some(eth_tx_data.to_vec());
                ...
                // Final fallback: use known size (without the script opcodes)
                let expected_tx_size = 44; // Adjust based on what we've seen (46 - 2 for OP_ENDIF OP_TRUE)
                let eth_tx_end = eth_tx_start + expected_tx_size;

                if eth_tx_end <= bytes.len() {
                    let eth_tx_data = &bytes[eth_tx_start..eth_tx_end];
                    println!(
                        "       ⚠️  Extracted {} bytes using fallback size",
                        eth_tx_data.len()
                    );
                    return Some(eth_tx_data.to_vec());
                }
    ...
    fn extract_envelope_data(&self, script: &Script) -> Option<Vec<u8>> {
        let mut instr = script.instructions();
        ...
        loop {
            match instr.next().and_then(|r| r.ok()) {
                Some(Instruction::Op(OP_ENDIF)) => break,
                Some(Instruction::PushBytes(b)) => data.extend_from_slice(b.as_bytes()),
                _ => return None,
            }
        }
